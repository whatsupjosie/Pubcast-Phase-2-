"""
vault_engine_hardened.py â€” Iteration Wallet Hardened Vault Engine
...
SECURITY LAYERS (defense in depth):
  ...
  6. Three-step physical confirmation: OPEN â†’ REMOVE â†’ DELETE
...
Rear View Foresight LLC â€” Feic Mo ChroÃ­ â€” 2026-03-24T06:00Z
"""
...
# â”€â”€â”€ OS-Level File Protection â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class OSProtection:
    ...
    def lock_file(path: Path) -> bool:
        ...
        try:
            if SYSTEM == "Darwin":
                # macOS: chflags uchg â€” user immutable
                ...
                # Linux: chattr +i â€” immutable flag
...
# â”€â”€â”€ Server-Side Command Enforcer â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

class CommandEnforcer:
    """
    Server-side keystroke validation.
    The frontend sends individual keystrokes with timestamps.
    The SERVER validates timing â€” no client-side trust.
    """
...
# â”€â”€â”€ Hardened Vault Engine â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class VaultEngine:
    ...
    def add_file_to_vault(self, source_path: str, project_name: str,
        ...
        if ".." in str(source) or ".." in project_name:
            return False, "Path traversal detected â€” operation denied."
        ...
        return True, f"âœ“ {source.name} protected in Vault with OS immutability."
    ...
    def remove_file_from_vault(self, file_id: str,
        ...
        return True, "âœ“ File removed from Vault. Use DELETE to permanently destroy."
    ...
    def delete_file(self, file_id: str, session_id: str) -> Tuple[bool, str]:
        ...
        return True, "âœ“ File permanently deleted. No recovery possible."

    # â”€â”€â”€ Cradle Management â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def create_cradle(self, project_name: str,
        ...
        return True, f"âœ“ Cradle created for '{project_name}'"
    ...
    def promote_version(self, project_name: str,
        ...
        self._log_event("VERSION_PROMOTED", None,
                        f"'{project_name}' promoted {current_version} â†’ {new_version}")
        return True, f"âœ“ {current_version} â†’ {new_version}. Old version archived."

    # â”€â”€â”€ File Watcher with Shadow Restore â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def _verify_and_restore(self):
        ...
        for f in files:
            ...
            if not vault_path.exists():
                # FILE WAS DELETED â€” restore from shadow
            ...
            if current_cs != f["checksum"]:
                # FILE WAS MODIFIED â€” revert from shadow
                self._log_event("INTEGRITY_VIOLATION", f["file_id"],
            ...
            if shadow_path and shadow_path.exists():
                ...
                if shadow_cs != f["checksum"]:
                    # Shadow was tampered â€” restore from vault
                    OSProtection.unlock_file(shadow_path)
                    shutil.copy2(vault_path, shadow_path)
                    OSProtection.lock_file(shadow_path)
            elif shadow_path and not shadow_path.exists():
                # Shadow was deleted â€” recreate from vault
    ...
    def _checksum(self, path: Path) -> str:
        ...
        try:
            return _read_checksum()
        except (PermissionError, OSError):
            # On Windows, the vault's own protection can block the service from
            # re-reading protected files during integrity checks. Temporarily
            # lift protection, read the bytes, then re-apply protection.
            if SYSTEM == "Windows" and path.exists():
                unlocked = OSProtection.unlock_file(path)
                try:
                    return _read_checksum()
                except (PermissionError, OSError):
                    return ""
